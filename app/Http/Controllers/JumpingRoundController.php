...
public function edit(JumpingRound $jumping_round){
        ...
        $this->log_to_db($jumping_round,"edit");
    ...
    private function log_to_db(JumpingRound $jumping_round,string $action, $comment = null)
{

    $usage_log = new UsageLogController();
    $usage_log->save($jumping_round,$action,$comment);
}
    ...
    private function generateLog($round){
        ...
        $message = json_encode($round);
        //dd($message);

        $this->log_to_db($round,"update",$message);
