...
class PKDressage2OutOf3Controller extends Controller

{
    ...
    //dd($uniqueStarts->sortBy("rider_name")); 
        ...
        foreach($uniqueStarts as $start){
            ...
            if (count($foundStarts["starts"])>=$numberOfEvents-1) $withAllStarts[]=$foundStarts;
    ...
    private function getAllStarts(array $startsArray, Start $start){
        ...
        $collection = $collection->sortByDesc("percent");
        $avg=$collection->where("completed",">",0)->take(2)->avg("percent") ?? 0;
